...
import dictionary.DictionaryZmeul;
import utils.tokenizer.TokenizerZmeul;
...
public class IndexerZmeul {
    public static void index(String[] filePaths, BooleanDictionary<?> dictionary, TokenizerZmeul tokenizer) {
        for (String path : filePaths) {
            indexSingleFile(path, dictionary, tokenizer);
        }
    }

    private static void indexSingleFile(String filePath, BooleanDictionary<?> dictionary, TokenizerZmeul tokenizer) {
    ...
    public static void indexSingleFile(String filePath, DictionaryZmeul dictionary) {
        File f = new File(filePath);
        if (!f.exists()) {
            System.err.println("File not found: " + filePath);
            return;
